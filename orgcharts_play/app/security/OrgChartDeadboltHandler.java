...
//		doBuildFakeLoginUser(context.session(), user);
		OrgChartUser user = getOrgChartUserByContext(context);
	...
	public static OrgChartUser getOrgChartUserByContext(Context context) {
		Session session = context.session();
		if(session.containsKey("fakelogin")){
			OrgChartUser user = new OrgChartUser(new UserInfo());
			doBuildFakeLoginUser(session, user);
			Logger.info("@getOrgChartUserByContext doBuildFakeLoginUser: "+user);
			return user;
		}else{
			CookieManager cookieman = new CookieManager();
			cookieman.injectCookies(context.request().cookies());
			UserInfo userInfo = new UserInfo();
			userInfo.inputCookies(cookieman);
			if(null!=userInfo&&null!=userInfo.getHinuser()&&false==userInfo.getHinuser().equals("")){
				OrgChartUser user = new OrgChartUser(userInfo);
//				Logger.info("@getOrgChartUserByContext OrgChartUser="+user);
				return user;
			}else{
				Logger.debug("@getOrgChartUserByContext: Cookies input failed: userInfo="+userInfo);
				return null;
			}
		}
	}
