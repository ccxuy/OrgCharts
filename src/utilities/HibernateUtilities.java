...
public class HibernateUtilities {
	...
	private static SessionFactory sessfactory;
	 ...
	 public static List searchResults() throws Exception {
	    ...
	    public static void addRecord(String sfirstName, String slastName, String email, Blob dataBlob) {
	        Session session = sessfactory.openSession();
	        Transaction tx = null;
	        try {
	            tx = session.beginTransaction();
	            ProfileBean data = new ProfileBean( sfirstName, slastName, email, dataBlob);	            
	            session.save(data);
	            tx.commit();
	    ...
	    public static void editNode(String sfirstName, String slastName, String email, Blob image, int empId) {
	        Session session = sessfactory.openSession();
	        Query query = null;
	        Transaction tx = null;
	        //String hql = "update ProfileBean set name = :sname where node = :empnode";
	    ...
	    public static List searchId() throws Exception {
	        ...
	        try {
	            tx = session.beginTransaction();
	            query1 = session.createQuery("select id from ProfileBean order by id");
	            query2 = session.createQuery("select firstName from ProfileBean order by id");
	            query3 = session.createQuery("select lastName from ProfileBean order by id");
	            id = query1.list();
	            first = query2.list();
	            last = query3.list();
	            for(int x=0; x<id.size(); x++){
	            	toAdd=id.get(x).toString()+":"+first.get(x).toString()+":"+last.get(x).toString();
	            	data.add(toAdd);
	            }
	            tx.commit();
	        } catch (HibernateException e) {
...
//////////////////////////////////////	    
	    public static List getEditedData(String id ) {
	        ...
	        try {
	            ...
	            //query = session.createQuery("select firstName, lastName, email, img from ProfileBean where id = :id");
	            //query.setParameter("id", Integer.parseInt(id));
	            System.out.println("in here");
	            query1 = session.createQuery("select firstName from ProfileBean where id= :id");
	            query2 = session.createQuery("select lastName from ProfileBean where id= :id");
	            query3 = session.createQuery("select email from ProfileBean where id= :id");
	            query4 = session.createQuery("select img from ProfileBean where id= :id");
